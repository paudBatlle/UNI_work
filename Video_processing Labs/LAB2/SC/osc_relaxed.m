...
elseif (filter==2)
    %diagonal de 1s
    %(triu(ones(xn,xn-1),-1) - triu(ones(xn, xn-1),1))+ (triu(ones(xn, xn-1))-triu(ones(xn, xn-1),-1))
    R=zeros(xn,xn);   %<-------- INCLUDE YOUR SECOND-ORDER FILTER
    R = diag([1,ones(1,xn-2)*2,1]) + diag(-1*ones(1,xn-1),1)+diag(-1*ones(1,xn-1),-1);
elseif (filter==4)    
    R=zeros(xn,xn);   %<-------- INCLUDE YOUR FOURTH-ORDER FILTER
    R = diag([0,-16,ones(1,xn-3)*(-30),-16,0],0) + diag([1,ones(1,xn-2)*16,1],1) + diag([1,ones(1,xn-2)*16,1],-1) + diag([-1*ones(1,xn-1)],2) + diag([-1*ones(1,xn-1)],-2);
...
% OPTIMIZATION BLOCK
if (filter == 1)             %si es filtro = 1
    U = zeros(xn, xn-1);
    U_prev = U;
    Y_2 = zeros(xn, xn-1);
elseif (filter == 2)        %si es filtro = 2
    U = zeros(xn, xn);
    U_prev = U;
    Y_2 = zeros(xn, xn);
end
